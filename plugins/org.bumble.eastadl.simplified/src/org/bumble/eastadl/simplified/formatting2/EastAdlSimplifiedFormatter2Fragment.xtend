...
class EastAdlSimplifiedFormatter2Fragment extends AbstractStubGeneratingFragment {
	...
	protected def StringConcatenationClient generateFormatMethod(EClass clazz, Collection<EReference> containmentRefs, boolean isOverriding) '''
		«IF isOverriding»override«ELSE»def«ENDIF» dispatch void format(«clazz» «clazz.toVarName», extension «IFormattableDocument» document) {
			val open = «clazz.toVarName».regionFor.keyword('{')
			val close = «clazz.toVarName».regionFor.keyword('}')
			
			open.append[newLine]
			close.prepend[newLine].append[newLine]
			
			for (attr : «clazz.toVarName».eClass.EAllAttributes) {
				«/* Bit of a hack here to get the semantic region for the semicolon.  */»
				val attrRegion = «clazz.toVarName».regionFor.keyword(attr.name)
				if (attrRegion !== null) {
			...
			«FOR ref:containmentRefs»
				«IF ref.isMany»
					...
					«/* Bit of a hack here since "return" is an Xtend keyword, but also used in the Operation grammar rule.  */»
				«ELSEIF ref.getAccessor != "return"»
